...
class VanillaAgent(BaseAgent):
    ...
    async def process(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        content = generation_utils.strip_reasoning_trace(content)
        visual_intent = generation_utils.strip_reasoning_trace(data["visual_intent"])
        ...
        lower_model = (self.model_name or "").lower()
        ...
        elif any(token in lower_model for token in ["gpt-image", "nano-banana", "sora-image"]):
            aspect_ratio = "auto"
            image_size = "1K"
            if "additional_info" in data and isinstance(data["additional_info"], dict):
                aspect_ratio = data["additional_info"].get("rounded_ratio") or aspect_ratio
                image_size = data["additional_info"].get("image_size") or image_size
            image_config = {
                "size": "1536x1024",
        ...
        if not response_list or not response_list[0] or str(response_list[0]).strip().lower() == "error":
            raise ValueError(f"Image generation returned no usable image for model={self.model_name}")
