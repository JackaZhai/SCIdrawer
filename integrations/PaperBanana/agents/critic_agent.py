...
class CriticAgent(BaseAgent):
    ...
    async def process(self, data: Dict[str, Any], source: str = "stylist") -> Dict[str, Any]:
        ...
        revised_description = generation_utils.strip_reasoning_trace(revised_description)
        ...
        if revised_description.strip() == "No changes needed.":
            data[f"target_{task_name}_critic_desc{round_idx}"] = generation_utils.strip_reasoning_trace(detailed_description)
...
1. Content
    ...
    -   **Unit-Label Cleanup:** Remove unnecessary measurement-unit annotation text from visible labels (e.g., 1cm, 2px, 12pt, 5mm, 300dpi) unless the user explicitly requires such labels.
