...
class PaperVizProcessor:
    ...
    def __init__(
        ...
        progress_callback: Optional[Callable[[str, str, int], Optional[Awaitable[None]]]] = None,
    ):
    ...
    async def _emit_progress(self, stage: str, message: str, progress: int) -> None:
    ...
    async def _run_critic_iterations(self, data: Dict[str, Any], task_name: str, max_rounds: int = 3, source: str = "stylist") -> Dict[str, Any]:
        ...
        for round_idx in range(max_rounds):
            await self._emit_progress(
                "processing_critic",
                f"批评迭代第 {round_idx + 1}/{max_rounds} 轮",
                76,
            )
            ...
            await self._emit_progress("processing_visualizer", "根据批评建议重新生成图像", 74)
    ...
    async def process_single_query(
        ...
        if exp_mode == "vanilla":
            await self._emit_progress("processing_visualizer", "图像生成（image-only）", 72)
            ...
            print("[Pipeline] dev_full: retriever start")
            await self._emit_progress("processing_retriever", "检索参考样例", 48)
            data = await self.retriever_agent.process(data, retrieval_setting=retrieval_setting)
            print("[Pipeline] dev_full: retriever done")
            print("[Pipeline] dev_full: planner start")
            await self._emit_progress("processing_planner", "生成结构化图像规划", 56)
            data = await self.planner_agent.process(data)
            print("[Pipeline] dev_full: planner done")
            print("[Pipeline] dev_full: stylist start")
            await self._emit_progress("processing_stylist", "优化视觉风格描述", 64)
            data = await self.stylist_agent.process(data)
            print("[Pipeline] dev_full: stylist done")
            print("[Pipeline] dev_full: visualizer start")
            await self._emit_progress("processing_visualizer", "执行图像生成", 72)
            data = await self.visualizer_agent.process(data)
            print("[Pipeline] dev_full: visualizer done")
            # Use max_critic_rounds from data (if set) or config
            max_rounds = data.get("max_critic_rounds", self.exp_config.max_critic_rounds)
            print(f"[Pipeline] dev_full: critic start rounds={max_rounds}")
            if max_rounds > 0:
                await self._emit_progress("processing_critic", f"开始批评迭代，共 {max_rounds} 轮", 76)
            data = await self._run_critic_iterations(data, task_name, max_rounds=max_rounds, source="stylist")
            print("[Pipeline] dev_full: critic done")
            if "demo" in exp_mode:
                do_eval = False
        ...
        if do_eval:
            await self._emit_progress("processing_eval", "评估与结果整理", 82)
