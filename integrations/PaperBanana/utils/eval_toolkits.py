...
try:
    ...
    # Guard against "prompts" name collision with unrelated installed packages.
    _pb_root = Path(__file__).resolve().parents[1]
    _local_prompts_dir = _pb_root / "prompts"
    if str(_pb_root) not in sys.path:
        sys.path.insert(0, str(_pb_root))

    existing_prompts = sys.modules.get("prompts")
    existing_file = (getattr(existing_prompts, "__file__", "") or "").lower() if existing_prompts else ""
    if existing_prompts and str(_local_prompts_dir).lower() not in existing_file:
        sys.modules.pop("prompts", None)

    diagram_eval_prompts = importlib.import_module("prompts.diagram_eval_prompts")
    plot_eval_prompts = importlib.import_module("prompts.plot_eval_prompts")
...
def _extract_winner_with_fallback(clean_json: str, eval_dim: str, valid_winners: list[str]) -> str:
    ...
    if extracted and extracted in valid_winners:
        print(f"[WARN] {eval_dim}: regex extracted '{extracted}'")
        return extracted
    print(f"[WARN] {eval_dim}: failed to extract valid winner")
    return "Error"
...
async def get_score_for_image_referenced(
    ...
    if "path_to_gt_image" not in sample_data:
        print("[WARN] No ground truth image path found. Skipping evaluation.")
    ...
    if eval_image_field not in sample_data:
        print(f"[WARN] Image field '{eval_image_field}' not found. Model generation failed - counting as Human win.")
