...
from .ocr.local_cnocr import LocalCnOcr
...
class TextRestorer:
    ...
    @property
    def local_cnocr(self) -> LocalCnOcr:
        """Lazy init local cnocr backend."""
        if self._local_cnocr is None:
            self._local_cnocr = LocalCnOcr()
        return self._local_cnocr
    ...
    def _run_ocr(self, image_path: str):
        ...
        # 优先 Azure OCR；不可用时自动回退到本地 cnocr
        azure_result = None
        azure_error = None
        print("\n[OCR] Trying Azure OCR...")
        azure_start = time.time()
        try:
            azure_result = self.azure_ocr.analyze_image(image_path)
            self.timing["azure_ocr"] = time.time() - azure_start
            print(f"   Azure blocks: {len(azure_result.text_blocks)} ({self.timing['azure_ocr']:.2f}s)")
        except Exception as exc:
            azure_error = exc
            self.timing["azure_ocr"] = time.time() - azure_start
            print(f"   Azure OCR unavailable: {exc}")

        if azure_result is None:
            print("[OCR] Falling back to local cnocr...")
            local_start = time.time()
            try:
                azure_result = self.local_cnocr.analyze_image(image_path)
                self.timing["local_ocr"] = time.time() - local_start
                print(f"   Local OCR blocks: {len(azure_result.text_blocks)} ({self.timing['local_ocr']:.2f}s)")
            except Exception as local_exc:
                self.timing["local_ocr"] = time.time() - local_start
                raise RuntimeError(
                    f"All OCR backends failed. azure={azure_error}; local_cnocr={local_exc}"
                ) from local_exc
    ...
    def _save_metadata(self, image_path: str, output_path: str, text_blocks: List[Dict], 
        ...
        metadata = {
            ...
            "mode": f"ocr_auto+{self.formula_engine}",
            "timing": self.timing,
            "statistics": {
    ...
    def _print_stats(self, text_blocks: List[Dict]):
        ...
        print(f"   Local OCR: {self.timing['local_ocr']:.2f}s")
