...
class Sam3InfoExtractor(BaseProcessor):
    ...
    def __init__(self, config=None, checkpoint_path: str = None, bpe_path: str = None):
        ...
        # Resolve relative paths against Edit-Banana project root so runtime CWD does not matter.
        project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        if self._checkpoint_path and not os.path.isabs(self._checkpoint_path):
            self._checkpoint_path = os.path.abspath(os.path.join(project_root, self._checkpoint_path))
        if self._bpe_path and not os.path.isabs(self._bpe_path):
            self._bpe_path = os.path.abspath(os.path.join(project_root, self._bpe_path))
